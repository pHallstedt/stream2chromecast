...
def play(filename, transcode=False, transcoder=None, transcode_options=None, transcode_input_options=None,
    ...
    if os.path.isfile(filename):
        ...
        if transcode and (filename.lower().startswith("http://") or filename.lower().startswith(
                "https://") or filename.lower().startswith("rtsp://")):
    ...
    if subtitles:
        if os.path.isfile(subtitles):
            ...
            sub = "http://%s:%s?%s" % (
                webserver_ip, str(sub_server.server_port), quote_plus(subtitles, "/"))
            print("sub URL: " + sub)
        else:
